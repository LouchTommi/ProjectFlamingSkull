...
Shader "AE/Grunge"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline float4 TriplanarSampling107( sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
			{
				float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
				projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
				float3 nsign = sign( worldNormal );
						...
						)
			{
				...
				float2 texCoord124 = packedInput.ase_texcoord6.xy * _Tiling_Main_Texture + float2( 0,0 );
				float4 temp_cast_0 = (1.0).xxxx;
				float2 temp_cast_1 = (( 2.0 / _Grunge_Texture_Scale )).xx;
				float3 ase_positionWS = GetAbsolutePositionWS( PositionRWS );
				float4 triplanar122 = TriplanarSampling122( _Grunge_Texture, ase_positionWS, NormalWS, 1.0, temp_cast_1, 1.0, 0 );
				float temp_output_121_0 = ( 1.0 - _Grunge_Texture_Opacity );
				float4 temp_cast_2 = (temp_output_121_0).xxxx;
				float4 lerpResult126 = lerp( triplanar122 , temp_cast_2 , temp_output_121_0);
				#ifdef _GRUNGE_TEXTURE_ON_ON
				float4 staticSwitch133 = lerpResult126;
				#else
				float4 staticSwitch133 = _Grunge_Tint;
				#endif
				float2 temp_cast_4 = (( 2.0 / _GrungeScale )).xx;
				float4 triplanar107 = TriplanarSampling107( _Grunge_Mask, ase_positionWS, NormalWS, 1.0, temp_cast_4, 1.0, 0 );
				#if defined( _GRUNGECHANNEL_R )
				float staticSwitch110 = triplanar107.x;
				#elif defined( _GRUNGECHANNEL_G )
				float staticSwitch110 = triplanar107.g;
				#elif defined( _GRUNGECHANNEL_B )
				float staticSwitch110 = triplanar107.b;
				#else
				float staticSwitch110 = triplanar107.g;
				#endif
				float clampResult118 = clamp( (0.0 + (staticSwitch110 - _Grunge_Min) * (1.0 - 0.0) / (_Grunge_Max - _Grunge_Min)) , 0.0 , 1.0 );
				#ifdef _GRUNGE_ON_ON
				float staticSwitch127 = ( _Grunge_Opacity * clampResult118 );
				#else
				float staticSwitch127 = 0.0;
				#endif
				float4 lerpResult138 = lerp( temp_cast_0 , staticSwitch133 , staticSwitch127);
				
				float3 unpack140 = UnpackNormalScale( tex2D( _Normal, texCoord124 ), _Normal_Scale );
				unpack140.z = lerp( 1, unpack140.z, saturate(_Normal_Scale) );
				float3 lerpResult148 = lerp( unpack140 , float3(0,0,1) , ( _Grunge_Normal * staticSwitch127 ));
				
				float4 tex2DNode132 = tex2D( _Metallic_Smoothness, texCoord124 );
				
				float lerpResult147 = lerp( ( _Smoothness * tex2DNode132.a ) , _Grunge_Smoothness , staticSwitch127);
				
				surfaceDescription.BaseColor = ( ( _Tint * tex2D( _Base_Color, texCoord124 ) ) * lerpResult138 ).rgb;
				surfaceDescription.Normal = lerpResult148;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = tex2DNode132.r;
				...
				surfaceDescription.Smoothness = lerpResult147;
				surfaceDescription.Occlusion = tex2D( _AmbientOcclusion, texCoord124 ).r;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 texCoord124 = packedInput.ase_texcoord2.xy * _Tiling_Main_Texture + float2( 0,0 );
				float4 temp_cast_0 = (1.0).xxxx;
				float2 temp_cast_1 = (( 2.0 / _Grunge_Texture_Scale )).xx;
				float3 ase_positionWS = packedInput.ase_texcoord3.xyz;
				float3 ase_normalWS = packedInput.ase_texcoord4.xyz;
				float4 triplanar122 = TriplanarSampling122( _Grunge_Texture, ase_positionWS, ase_normalWS, 1.0, temp_cast_1, 1.0, 0 );
				float temp_output_121_0 = ( 1.0 - _Grunge_Texture_Opacity );
				float4 temp_cast_2 = (temp_output_121_0).xxxx;
				float4 lerpResult126 = lerp( triplanar122 , temp_cast_2 , temp_output_121_0);
				#ifdef _GRUNGE_TEXTURE_ON_ON
				...
				#endif
				float2 temp_cast_4 = (( 2.0 / _GrungeScale )).xx;
				float4 triplanar107 = TriplanarSampling107( _Grunge_Mask, ase_positionWS, ase_normalWS, 1.0, temp_cast_4, 1.0, 0 );
				#if defined( _GRUNGECHANNEL_R )
				float staticSwitch110 = triplanar107.x;
				#elif defined( _GRUNGECHANNEL_G )
				float staticSwitch110 = triplanar107.g;
		...
		Pass
		{
					...
					)
			{
				...
				float2 texCoord124 = packedInput.ase_texcoord4.xy * _Tiling_Main_Texture + float2( 0,0 );
				float3 unpack140 = UnpackNormalScale( tex2D( _Normal, texCoord124 ), _Normal_Scale );
				unpack140.z = lerp( 1, unpack140.z, saturate(_Normal_Scale) );
				float2 temp_cast_0 = (( 2.0 / _GrungeScale )).xx;
				float3 ase_positionWS = GetAbsolutePositionWS( PositionRWS );
				float4 triplanar107 = TriplanarSampling107( _Grunge_Mask, ase_positionWS, NormalWS, 1.0, temp_cast_0, 1.0, 0 );
				#if defined( _GRUNGECHANNEL_R )
				float staticSwitch110 = triplanar107.x;
				#elif defined( _GRUNGECHANNEL_G )
				float staticSwitch110 = triplanar107.g;
		...
		Pass
		{
				...
				)
			{
				...
				SmoothSurfaceDescription surfaceDescription = (SmoothSurfaceDescription)0;
				float2 texCoord124 = packedInput.ase_texcoord3.xy * _Tiling_Main_Texture + float2( 0,0 );
				float3 unpack140 = UnpackNormalScale( tex2D( _Normal, texCoord124 ), _Normal_Scale );
				unpack140.z = lerp( 1, unpack140.z, saturate(_Normal_Scale) );
				float2 temp_cast_0 = (( 2.0 / _GrungeScale )).xx;
				float3 ase_positionWS = packedInput.ase_texcoord4.xyz;
				float3 ase_normalWS = packedInput.ase_texcoord5.xyz;
				float4 triplanar107 = TriplanarSampling107( _Grunge_Mask, ase_positionWS, ase_normalWS, 1.0, temp_cast_0, 1.0, 0 );
				#if defined( _GRUNGECHANNEL_R )
				float staticSwitch110 = triplanar107.x;
				#elif defined( _GRUNGECHANNEL_G )
				float staticSwitch110 = triplanar107.g;
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 texCoord124 = packedInput.ase_texcoord8.xy * _Tiling_Main_Texture + float2( 0,0 );
				float4 temp_cast_0 = (1.0).xxxx;
				float2 temp_cast_1 = (( 2.0 / _Grunge_Texture_Scale )).xx;
				float3 ase_positionWS = GetAbsolutePositionWS( PositionRWS );
				float4 triplanar122 = TriplanarSampling122( _Grunge_Texture, ase_positionWS, NormalWS, 1.0, temp_cast_1, 1.0, 0 );
...
/*ASEBEGIN
Version=19801
Node;AmplifyShaderEditor.RangedFloatNode;102;-417.377,742.569;Inherit;False;Property;_GrungeScale;GrungeScale;11;0;Create;True;0;0;0;False;0;False;1;43;1;64;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;103;-289.377,550.569;Inherit;False;Constant;_Float0;Float 0;2;0;Create;True;0;0;0;False;0;False;2;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.WorldPosInputsNode;104;-321.377,134.569;Inherit;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SimpleDivideOpNode;105;-49.37695,470.569;Inherit;False;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;106;-385.377,358.569;Inherit;True;Property;_Grunge_Mask;Grunge_Mask;5;0;Create;True;0;0;0;False;0;False;8086b626d4688f84faef7403c4eec05e;8086b626d4688f84faef7403c4eec05e;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TriplanarNode;107;110.623,246.5689;Inherit;True;Spherical;World;False;Top Texture 0;_TopTexture0;white;-1;None;Mid Texture 0;_MidTexture0;white;-1;None;Bot Texture 0;_BotTexture0;white;-1;None;Triplanar Sampler;Tangent;10;0;SAMPLER2D;;False;5;FLOAT;1;False;1;SAMPLER2D;;False;6;FLOAT;0;False;2;SAMPLER2D;;False;7;FLOAT;0;False;9;FLOAT3;0,0,0;False;8;FLOAT;1;False;3;FLOAT2;1,1;False;4;FLOAT;1;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;108;462.623,-41.43097;Inherit;False;Property;_Grunge_Min;Grunge_Min;16;0;Create;True;0;0;0;False;0;False;0;0.022;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;109;462.623,54.56903;Inherit;False;Property;_Grunge_Max;Grunge_Max;15;0;Create;True;0;0;0;False;0;False;0.3000374;0.617;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;110;542.623,278.569;Inherit;False;Property;_GrungeChannel;GrungeChannel;13;0;Create;True;0;0;0;False;0;False;0;1;1;True;;KeywordEnum;3;R;G;B;Create;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;111;-305.377,-505.4315;Inherit;False;Constant;_Float2;Float 0;2;0;Create;True;0;0;0;False;0;False;2;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;112;-433.377,-313.4311;Inherit;False;Property;_Grunge_Texture_Scale;Grunge_Texture_Scale;20;0;Create;True;0;0;0;False;0;False;5.898302;12.6;1;64;0;1;FLOAT;0
Node;AmplifyShaderEditor.TFHCRemapNode;113;862.623,246.5689;Inherit;False;5;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;0;False;4;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;114;574.623,-505.4315;Inherit;False;Property;_Grunge_Texture_Opacity;Grunge_Texture_Opacity;21;0;Create;True;0;0;0;False;0;False;1;0.469;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleDivideOpNode;115;-65.37695,-585.4315;Inherit;False;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.WorldPosInputsNode;116;-194.97,-788.2834;Inherit;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.TexturePropertyNode;117;-49.37695,-969.4304;Inherit;True;Property;_Grunge_Texture;Grunge_Texture;4;0;Create;True;0;0;0;False;0;False;892df4ee12c65c743bef91e8b7a47532;892df4ee12c65c743bef91e8b7a47532;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.ClampOpNode;118;1070.623,246.5689;Inherit;True;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;119;926.623,134.569;Inherit;False;Property;_Grunge_Opacity;Grunge_Opacity;14;0;Create;True;0;0;0;False;0;False;0.6238509;0.69;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.Vector2Node;120;-433.377,918.569;Inherit;False;Property;_Tiling_Main_Texture;Tiling_Main_Texture;6;0;Create;True;0;0;0;False;0;False;1,1;1,1;0;3;FLOAT2;0;FLOAT;1;FLOAT;2
Node;AmplifyShaderEditor.OneMinusNode;121;830.623,-585.4315;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TriplanarNode;122;286.623,-841.4304;Inherit;True;Spherical;World;False;Top Texture 1;_TopTexture1;white;-1;None;Mid Texture 1;_MidTexture1;white;-1;None;Bot Texture 1;_BotTexture1;white;-1;None;Triplanar Sampler;Tangent;10;0;SAMPLER2D;;False;5;FLOAT;1;False;1;SAMPLER2D;;False;6;FLOAT;0;False;2;SAMPLER2D;;False;7;FLOAT;0;False;9;FLOAT3;0,0,0;False;8;FLOAT;1;False;3;FLOAT2;1,1;False;4;FLOAT;1;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;123;1293.03,155.7162;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TextureCoordinatesNode;124;-145.377,886.569;Inherit;False;0;-1;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;125;798.623,-345.4311;Inherit;False;Property;_Grunge_Tint;Grunge_Tint;12;0;Create;True;0;0;0;False;0;False;0.5566038,0.5566038,0.5566038,0;0.1465378,0.2395391,0.3490565,0;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.LerpOp;126;1038.623,-633.4315;Inherit;True;3;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;2;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.StaticSwitch;127;1581.03,171.7162;Inherit;True;Property;_Grunge_ON;Grunge_ON;10;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;128;270.623,470.569;Inherit;False;Property;_Tint;Tint;7;0;Create;True;0;0;0;False;0;False;1,1,1,0;0.7264151,0.7264151,0.7264151,0;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.RangedFloatNode;129;942.623,22.56903;Inherit;False;Constant;_Float1;Float 1;12;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;130;590.623,806.569;Inherit;False;Property;_Smoothness;Smoothness;8;0;Create;True;0;0;0;False;0;False;0.2765529;0.66;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;131;206.623,710.569;Inherit;True;Property;_Base_Color;Base_Color;0;0;Create;True;0;0;0;False;0;False;-1;4814fa37e4b2bdf4f8c893b57b2a05b6;4814fa37e4b2bdf4f8c893b57b2a05b6;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SamplerNode;132;190.623,918.569;Inherit;True;Property;_Metallic_Smoothness;Metallic_Smoothness;1;0;Create;True;0;0;0;False;0;False;-1;370c6a8f759f7c1409e438d8e9cdb247;370c6a8f759f7c1409e438d8e9cdb247;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.StaticSwitch;133;1342.623,-457.4315;Inherit;True;Property;_Grunge_Texture_ON;Grunge_Texture_ON;19;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;134;-114.97,1179.716;Inherit;False;Property;_Normal_Scale;Normal_Scale;9;0;Create;True;0;0;0;False;0;False;1;0.97;0;15;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;135;1344.748,755.7385;Inherit;True;Property;_Grunge_Normal;Grunge_Normal;18;0;Create;True;0;0;0;False;0;False;5;1.45;0;1.45;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;136;638.623,582.569;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;137;1069.03,651.7162;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;138;1709.03,-132.2838;Inherit;True;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;139;1133.03,555.7162;Inherit;False;Property;_Grunge_Smoothness;Grunge_Smoothness;17;0;Create;True;0;0;0;False;0;False;0.2765529;0.907;-1;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;140;192.748,1123.739;Inherit;True;Property;_Normal;Normal;2;0;Create;True;0;0;0;False;0;False;-1;f62c523b81b614147b075badd08f49a2;101988d3209fab6489afa98fa66e2e81;True;0;True;bump;Auto;True;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;141;1712.748,755.7385;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector3Node;142;1800.567,1075.721;Inherit;True;Constant;_Vector0;Vector 0;22;0;Create;True;0;0;0;False;0;False;0,0,1;0,0,0;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.BreakToComponentsNode;143;683.6541,246.6177;Inherit;False;FLOAT;1;0;FLOAT;0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
Node;AmplifyShaderEditor.BreakToComponentsNode;144;435.9564,193.6459;Inherit;False;FLOAT;1;0;FLOAT;0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
Node;AmplifyShaderEditor.SamplerNode;145;189.03,1339.716;Inherit;True;Property;_AmbientOcclusion;Ambient Occlusion;3;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;146;1997.03,-68.28375;Inherit;True;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.LerpOp;147;1840.748,515.7385;Inherit;True;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;148;2016.748,755.7385;Inherit;True;3;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;37;2304,272;Float;False;True;-1;2;Rendering.HighDefinition.LightingShaderGraphGUI;0;14;AE/Grunge;53b46d85872c5b24c8f4f0a1c3fe4c87;True;GBuffer;0;0;GBuffer;34;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;3;RenderPipeline=HDRenderPipeline;RenderType=Opaque=RenderType;Queue=Geometry=Queue=0;True;5;True;7;d3d11;metal;vulkan;xboxone;xboxseries;playstation;switch;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;0;True;_CullMode;False;True;True;True;True;True;0;True;_LightLayersMaskBuffer4;False;False;False;False;False;False;False;True;True;0;True;_StencilRefGBuffer;255;False;;255;True;_StencilWriteMaskGBuffer;7;False;;3;False;;0;False;;0;False;;7;False;;3;False;;0;False;;0;False;;False;False;True;0;True;_ZTestGBuffer;False;True;1;LightMode=GBuffer;False;False;0;;0;0;Standard;38;Surface Type;0;0;  Rendering Pass;1;0;  Refraction Model;0;0;    Blending Mode;0;0;    Blend Preserves Specular;1;0;  Back Then Front Rendering;0;0;  Transparent Depth Prepass;0;0;  Transparent Depth Postpass;0;0;  ZWrite;0;0;  Z Test;4;0;Double-Sided;0;0;Alpha Clipping;0;0;  Use Shadow Threshold;0;0;Material Type,InvertActionOnDeselection;0;0;  Energy Conserving Specular;1;0;  Transmission,InvertActionOnDeselection;0;0;Receive Decals;1;0;Receive SSR;1;0;Receive SSR Transparent;0;0;Motion Vectors;1;0;  Add Precomputed Velocity;0;0;Specular AA;0;0;Specular Occlusion Mode;1;0;Override Baked GI;0;0;Depth Offset;0;0;  Conserative;1;0;GPU Instancing;1;0;LOD CrossFade;0;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,;0;  Type;0;0;  Tess;16,False,;0;  Min;10,False,;0;  Max;25,False,;0;  Edge Length;16,False,;0;  Max Displacement;25,False,;0;Vertex Position;1;0;0;11;True;True;True;True;True;True;False;False;False;True;True;False;;False;0
...
WireConnection;122;9;116;0
WireConnection;122;3;115;0
...
WireConnection;37;1;148;0
WireConnection;37;4;132;1
WireConnection;37;7;147;0
WireConnection;37;8;145;0
ASEEND*/
//CHKSM=15FA28B7D8D6699DF4BEE9779AE9C02E581337F1